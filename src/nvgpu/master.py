...
from nvgpu.list_gpus import device_table, format_table
...
def format_reports_to_ansi(hosts, reports):
    def format_report(host, report):
        if "error" not in report:
            return "\n".join(
                [
                    "%s (%s)" % (host, report["hostname"]),
                    format_table(device_table(report["gpus"])),
                ]
            )
        else:
    ...
    return "\n\n".join(
        [format_report(host, report) for host, report in zip(hosts, reports)]
    )
